...
function populateMainPlayer(docItem) {
    ...
    if (ytId) {
        ...
        iframe.style.height = '100%';
        target.appendChild(iframe);
        _attachPlayerControls(target, iframe, docItem);
...
function _attachPlayerControls(container, iframe, docItem) {
    // Remove any existing overlay
    const existing = container.querySelector('.player-controls-overlay');
    if (existing) existing.remove();
    ...
    // Helpers to send commands to the YouTube iframe
    ...
    // Track play state locally (YouTube postMessage doesn't return state)
    let isPlaying = true; // starts as playing (autoplay=1)
    const playIcon = 'fa-pause';
    const pauseIcon = 'fa-play';

    // Prev button
    const prevBtn = document.createElement('button');
    prevBtn.className = 'pco-btn';
    prevBtn.type = 'button';
    prevBtn.setAttribute('aria-label', 'Previous video');
    prevBtn.innerHTML = '<i class="fas fa-step-backward"></i>';
    prevBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        _playAdjacentVideo(docItem, -1);
    });

    // Play/Pause button
    const playBtn = document.createElement('button');
    playBtn.className = 'pco-btn';
    playBtn.type = 'button';
    ...
    playBtn.addEventListener('click', (e) => {
        ...
        if (isPlaying) {
            ytCmd('pauseVideo');
            playBtn.querySelector('i').className = `fas ${pauseIcon}`;
            container.classList.add('controls-visible');
        } else {
            ytCmd('playVideo');
            playBtn.querySelector('i').className = `fas ${playIcon}`;
            container.classList.remove('controls-visible');
        }
        isPlaying = !isPlaying;
    });

    // Listen for YouTube state changes to sync the icon
    window.addEventListener('message', function onYTMessage(ev) {
        let data;
    ...
    volGroup.className = 'pco-volume-group';

    const volIcon = document.createElement('button');
    volIcon.className = 'pco-btn';
    volIcon.type = 'button';
    volIcon.setAttribute('aria-label', 'Mute / Unmute');
    volIcon.innerHTML = '<i class="fas fa-volume-high"></i>';
    let muted = false;
    volIcon.addEventListener('click', (e) => {
        e.stopPropagation();
        muted = !muted;
        ytCmd(muted ? 'mute' : 'unMute');
        volIcon.querySelector('i').className = `fas ${muted ? 'fa-volume-xmark' : 'fa-volume-high'}`;
        if (!muted) ytCmd('setVolume', [parseInt(volSlider.value)]);
    });

    const volSlider = document.createElement('input');
    volSlider.type = 'range';
    volSlider.className = 'pco-volume-slider';
    volSlider.min = 0;
    volSlider.max = 100;
    volSlider.value = 80;
    ytCmd('setVolume', [80]);
    volSlider.addEventListener('input', (e) => {
        ...
        if (v === 0) {
            muted = true;
            ytCmd('mute');
            volIcon.querySelector('i').className = 'fas fa-volume-xmark';
        } else {
            muted = false;
            ytCmd('unMute');
            volIcon.querySelector('i').className = 'fas fa-volume-high';
        }
    });
...
function _playAdjacentVideo(currentDoc, direction) {
    if (!currentPlaylist || currentPlaylist.length === 0) return;
    const idx = currentPlaylist.findIndex(d => d.id === currentDoc.id);
    if (idx === -1) return;
    const nextIdx = (idx + direction + currentPlaylist.length) % currentPlaylist.length;
    playWatchVideo(currentPlaylist[nextIdx]);
}
